...
| 2 | Press key 2 | Left reboots (~2 s); Vision Pro sees "Halcyon Ferris" for pairing on profile 1; right half follows the left; dongle log shows standalone and it holds no peripheral | |
...
| 11 | Key 2 with the dongle unplugged or out of range while in dongle mode | Nothing happens (the left runs no keymap in dongle mode); expected | |
| 12 | Left half in dongle mode, before the dongle is learned: bonded Mac nearby | Log shows "Dropping host" for the Mac and "Recording <addr> as the dongle" naming the dongle, never the Mac | |
| 13 | Standalone → dongle: time from key 1 to the right typing through the dongle | Under 2 s. 4–6 s means the left reconnected the right before rebooting | |
| 14 | Known limitations to record: a settings_reset of the dongle changes its identity, so the left must be reset too; the right's two bonds are permanent, replacing either central needs a settings_reset on the right | acknowledged | |
